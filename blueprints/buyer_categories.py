...
buyer_categories_bp = Blueprint('buyer_categories', __name__)

# Constants for product image folders
PRODUCT_MAIN_PIC_FOLDER = "static/uploads/seller/product_main_pics"

def get_products_by_category(category, min_price=0, max_price=0, sort='recent', popularity='', brand=''):
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)

    # Base query with additional fields for popularity metrics
    query = """
        SELECT 
            pi.product_info_id,
            pi.product_category,
    ...
    if brand:
        query += " AND pi.product_name LIKE %s"
        filters.append(f"%{brand}%")
    
    query += " GROUP BY pi.product_info_id"

    # Handle sorting based on popularity or date
    if popularity == 'top_selling':
        query += " ORDER BY total_orders DESC, p.date_added DESC"
    elif popularity == 'most_reviewed':
        query += " ORDER BY total_feedback DESC, p.date_added DESC"
    elif popularity == 'highest_rated':
        query += " ORDER BY avg_rating DESC, total_ratings DESC, p.date_added DESC"
    elif sort == 'recent':
...
@buyer_categories_bp.route('/toggle_like_product/<int:product_info_id>', methods=['POST'])
...
@buyer_categories_bp.route('/categories/<category>', methods=['GET'])
def category_page(category):
    ...
    all_products = get_products_by_category(formatted_category, min_price, max_price, sort, popularity, brand)
    ...
    return render_template('buyer_categories.html', 
                           products=products, 
...
@buyer_categories_bp.route('/buyer/uploads/product_main_pics/<filename>')
