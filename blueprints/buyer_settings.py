...
from blueprints.get_user_info import get_personal_info, get_address_info, get_contact_info, get_valid_info
...
buyer_settings_bp = Blueprint('buyer_settings', __name__)

# Constants for file uploads
PROFILE_PIC_FOLDER = 'static/uploads/buyer/profile_pics'
VALID_ID_FOLDER = 'static/uploads/buyer/valid_id_pic'
ALLOWED_EXTENSIONS = {'png', 'jpg', 'jpeg'}

# Ensure folders exist
for folder in [PROFILE_PIC_FOLDER, VALID_ID_FOLDER]:
...
@buyer_settings_bp.route('/buyer/settings')
def settings():
    ...
    personal_info = get_personal_info(buyer_id)
    address_info = get_address_info(buyer_id)
    contact_info = get_contact_info(buyer_id)
    valid_info = get_valid_info(buyer_id)
    
    return render_template('buyer_settings.html', 
                         personal_info=personal_info,
                         address_info=address_info,
                         contact_info=contact_info,
                         valid_info=valid_info)

@buyer_settings_bp.route('/buyer/settings/update-personal', methods=['POST'])
def update_personal_info():
    ...
    try:
        ...
        cursor.execute("SELECT personal_id FROM user_account WHERE user_id = %s", (buyer_id,))
...
@buyer_settings_bp.route('/buyer/settings/update-address', methods=['POST'])
def update_address_info():
    ...
    try:
        ...
        cursor.execute("SELECT address_id FROM user_account WHERE user_id = %s", (buyer_id,))
        result = cursor.fetchone()
...
@buyer_settings_bp.route('/buyer/settings/update-contact', methods=['POST'])
def update_contact_info():
    ...
    try:
        ...
        cursor.execute("SELECT contact_id FROM user_account WHERE user_id = %s", (buyer_id,))
        result = cursor.fetchone()
...
@buyer_settings_bp.route('/buyer/settings/update-valid-id', methods=['POST'])
def update_valid_id():
    if 'buyer' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized access'})
    ...
    try:
        ...
        cursor.execute("SELECT valid_id FROM user_account WHERE user_id = %s", (buyer_id,))
        result = cursor.fetchone()
...
@buyer_settings_bp.route('/buyer/settings/change-password', methods=['POST'])
def change_password():
    if 'buyer' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized access'})
...
@buyer_settings_bp.route('/buyer/settings/update-profile-pic', methods=['POST'])
def update_profile_pic():
    ...
    try:
        ...
        cursor.execute("SELECT profile_pic FROM user_account WHERE user_id = %s", (buyer_id,))
        result = cursor.fetchone()
...
@buyer_settings_bp.route('/buyer/settings/profile_pics/<filename>')
def serve_profile_pic(filename):
    return send_from_directory(PROFILE_PIC_FOLDER, filename)

@buyer_settings_bp.route('/buyer/settings/valid_ids/<filename>')
