...
courier_settings_bp = Blueprint('courier_settings', __name__)

# Constants for file uploads
PROFILE_PIC_FOLDER = 'static/uploads/courier/profile_pics'
VALID_ID_FOLDER = 'static/uploads/courier/valid_id_pic'
...
@courier_settings_bp.route('/courier/settings')
def settings():
    ...
    personal_info = get_personal_info(courier_id)
    address_info = get_address_info(courier_id)
    contact_info = get_contact_info(courier_id)
    valid_info = get_valid_info(courier_id)
...
@courier_settings_bp.route('/courier/settings/update-personal', methods=['POST'])
def update_personal_info():
    ...
    try:
        ...
        cursor.execute("SELECT personal_id FROM user_account WHERE user_id = %s", (courier_id,))
...
@courier_settings_bp.route('/courier/settings/update-address', methods=['POST'])
def update_address_info():
    ...
    try:
        ...
        cursor.execute("SELECT address_id FROM user_account WHERE user_id = %s", (courier_id,))
        result = cursor.fetchone()
...
@courier_settings_bp.route('/courier/settings/update-contact', methods=['POST'])
def update_contact_info():
    ...
    try:
        ...
        cursor.execute("SELECT contact_id FROM user_account WHERE user_id = %s", (courier_id,))
        result = cursor.fetchone()
...
@courier_settings_bp.route('/courier/settings/update-valid-id', methods=['POST'])
def update_valid_id():
    if 'courier' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized access'})
    ...
    try:
        ...
        cursor.execute("SELECT valid_id FROM user_account WHERE user_id = %s", (courier_id,))
        result = cursor.fetchone()
...
@courier_settings_bp.route('/courier/settings/change-password', methods=['POST'])
def change_password():
    if 'courier' not in session:
        return jsonify({'success': False, 'message': 'Unauthorized access'})
...
@courier_settings_bp.route('/courier/settings/update-profile-pic', methods=['POST'])
def update_profile_pic():
    ...
    try:
        ...
        cursor.execute("SELECT profile_pic FROM user_account WHERE user_id = %s", (courier_id,))
        result = cursor.fetchone()
...
@courier_settings_bp.route('/courier/settings/profile_pics/<filename>')
def serve_profile_pic(filename):
    return send_from_directory(PROFILE_PIC_FOLDER, filename)

@courier_settings_bp.route('/courier/settings/valid_ids/<filename>')
